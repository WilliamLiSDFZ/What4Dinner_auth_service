...
**Two authentication flows produce the same final JWT** (60-min, claims: `iss=what4dinner-auth`, `sub=userId`, `email`):
...
**Family scoping**: every new user gets a brand-new `family` row (`UserInfoServiceImpl` creates it inside the same `@Transactional` method that inserts the user, name `default family name(please change)`). `family_id` is deliberately **not** a JWT claim — family membership is mutable, tokens last 60 minutes and cannot be revoked, so a baked-in family id would keep authorizing the family a user just left. Consumers read `users.family_id` directly (the whole platform shares one Postgres, so it is a PK lookup) or call `GET /v1/me`, which returns the caller's current `userId`/`email`/`username`/`familyId`. Do not "optimize" this back into the token.
